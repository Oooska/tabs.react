...
/* Tabs provides a simple way of providing tabbed components. 
  ...
  Multiple tabs being active is supported.
	Modify the property 'allowMultiple' to true to enable. 
  ...
  Props:
	...
	active: The current active tab(s) by title (or key if useKeys=true). 
	    If allowMultiple=true, active may be an array f titles/keys	
	
	useState: By default, Tabs maintains the active tab, and always allows the 
		user to switch tabs. if useState==true, the user is responsible for 
		maintaining prop 'active' based on the onChange callback.
	
	onChange: The callback that is called when the user clicks on a new tab. It
		provides the tabName or key of the tab clicks. If allowMultiple is true,
		onCHange will provide an array of the active tabNames/keys.
